...
static bool is_triangle(const Mesh& mesh, FaceH fh)
{
  if (!fh.is_valid()) {
    return false;
  }
...
static void lock_edges(Mesh& mesh)
{
  for (EdgeH eh : mesh.edges()) {
    auto he  = mesh.halfedge_handle(eh, 0);
    auto ohe = mesh.opposite_halfedge_handle(he);
    mesh.status(eh).set_locked(!is_triangle(mesh, mesh.face_handle(he)) ||
                               !is_triangle(mesh, mesh.face_handle(ohe)));
  }
}

static void unlock_edges(Mesh& mesh)
{
  for (EdgeH eh : mesh.edges()) {
